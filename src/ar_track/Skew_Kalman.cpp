...
//____prendo la misura corrispondente al centro dell'oggetto
		...
		q_k[3] = Transform_estimate->transform.rotation.w; 	//z		
		q_k[0] = Transform_estimate->transform.rotation.x;		//w	
		q_k[1] = Transform_estimate->transform.rotation.y;		//x
		q_k[2] = Transform_estimate->transform.rotation.z;		//y	
...
//		q[0] w q[1] x  q[2] y  q[3] z
		...
		q_k_dot(0) = ( q_k(0) - q_k_1(0) ) / dt ;   //z
		q_k_dot(1) = ( q_k(1) - q_k_1(1) ) / dt ;   //w
		q_k_dot(2) = ( q_k(2) - q_k_1(2) ) / dt ;  //x
		q_k_dot(3) = ( q_k(3) - q_k_1(3) ) / dt ;  //y
...
int main(int argc, char** argv)
{
	...
	ros::init(argc, argv, "skew_Kalman");// ROS node
   ros::NodeHandle nn;
	...
	pose_sub = nn.subscribe("transform_KALMAN_estimation", 1, &skew);
